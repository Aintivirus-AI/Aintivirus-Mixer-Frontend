...
export default function Page() {
    ...
    const handleConfirmDeposit = async () => {
        ...
        try {
            ...
            const res_1 = await MixAction.depositOnSolana(Number(amount), currenciesMap.solana[selectedCurrency], wallet.publicKey.toString());
    ...
    const executeJsonTransaction = async (
        jsonTx: any,
                    ...
                    >
                        <Tab key="deposit" title="Deposit">
                            <div className="flex flex-col gap-4">
                                <Select className="w-full" label="Currency" placeholder="Select a currency" selectedKeys={[selectedCurrency]}>
                                    {currencies.map((currency) => (
                                        <SelectItem key={currency.key} onPress={() => handleSelectCurrency(currency.key)}>{currency.label}</SelectItem>
                                    ))}
                                </Select>
                                <Select className="w-full" label="Amount" placeholder="Select an amount" selectedKeys={[amount]}>
                                    {(selectedCurrency === "sol" ? amountsSol : amountsToken).map((amount) => (
                                        <SelectItem key={amount.key} onPress={() => handleSelectAmount(amount.key)}>{amount.label}</SelectItem>
                                    ))}
                                </Select>
                                {/* <Input
                                    label="Amount"
                        ...
                        <Tab key="withdraw" title="Withdraw">
                            <div className="flex flex-col gap-4 h-[300px]">
                                <Input className="cursor-pointer" type="file" accept=".secret" label="Select your note.secret file" onChange={handleReadNoteFile} />
                                <Input required label="Recipient Address" placeholder="Enter recipient address" value={recipientAddress} onChange={(e) => setRecipientAddress(e.target.value)} />
                                <CustomConnectButton />
                                <Button color="primary" isLoading={loading} onPress={handleConfirmWithdraw} >Confirm Withdraw</Button>
                            </div>
                        </Tab>
                    </Tabs>
                </CardBody>
