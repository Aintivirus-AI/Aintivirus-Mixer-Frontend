...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html suppressHydrationWarning lang="en">
      <head />
      <body
        className={clsx('min-h-screen bg-black-50 font-sans antialiased dark', calSans.variable, fontSans.variable)}
      >
        <Providers>
              ...
              <div className="relative flex h-[100px] w-full justify-between lg:h-[110px]">
                {Array.from({ length: 3 }).map((_, index) => (
                  <div key={index} className="relative h-full flex-1">
                    <Image fill alt="frame" objectFit="cover" src="/frame.png" />
                  </div>
                ))}
              </div>
              <Navbar />
