...
export default class MixAction {
    static async depositOnEthereum(amount: string, currency: string, sender: string): Promise<{ data: { sessionId: string, expiresAt: number, transactions: Array<TransactionRequest> } }> {
        try {
    ...
    static async depositOnSolana(amount: number, currency: string, sender: string): Promise<{ data: { sessionId: string, expiresAt: number, transaction: Array<Transaction> } }> {
        try {
